...
function BattlePrint.PrintCharacter(battle)
	...
	for i = 1, len do
		...
		if hero then
			...
			dataStr[i] = dataStr[i] .. 'hp:' .. hero.data.hp .. ' atk:' .. hero.data.atk .. ' shield:' ..
				hero.data.shield
		...
		if mon then
			...
			empty = FillEmpty(num) --print(num)	
			dataStr[i] = dataStr[i] .. empty .. 'hp:' .. mon.data.hp ..
				' atk:' .. mon.data.atk .. ' shield:' .. mon.data.shield
...
function BattlePrint.PrintCard(battle)
	...
	local battleData = '行動點數: ' ..
		battle.battleData.actPoint ..
		'  牌堆: ' .. #battle.battleData.deck .. '  棄牌堆: ' .. #battle.battleData.drop ..
		'  消失牌堆: ' .. #battle.battleData.disappear
	...
	for k, card in pairs(battle.battleData.hand) do
		local card_name = _G.Resource.translate[card.key]['name']
		hand = hand .. 'c' .. k .. ' '
		if card == currentCard then
			hand = hand .. card_name .. '(cost:' .. card.cost .. ')(已選擇)'
		else
			hand = hand .. card_name .. '(cost:' .. card.cost .. ')'
		end
	...
	local info = {}
	for k, card in pairs(battle.battleData.hand) do
		local card_info = _G.Resource.translate[card.key]['info']

		info[k] = StringRead.StrPrintf(card_info, card, battle)
